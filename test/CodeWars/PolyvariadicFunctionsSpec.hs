...
module CodeWars.PolyvariadicFunctionsSpec (spec) where

import Test.Hspec
import Test.QuickCheck

import CodeWars.PolyvariadicFunctions
...
spec = do
  describe "polyAdd" $ do
    ...
    it "Works for QuickCheck tests" $
      property (\a b c d e -> (polyAdd a b c d e :: Int) === (a + b + c + d + e))
  ...
  describe "polyWords" $ do
    ...
    it "Works for QuickCheck tests" $
      property (\a b -> a /= "" && b /= "" ==> (polyWords a b ::String) === (a ++ " " ++ b))
  ...
  describe "polyList" $ do
    ...
    it "Works for QuickCheck tests" $
      property (\a b c -> (polyList a b c :: [Int]) === [a,b,c])
      .&&. property (\a b c d -> (polyList a b c d :: [Bool]) === [a,b,c,d])